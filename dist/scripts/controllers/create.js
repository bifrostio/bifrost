...
(function () {
  ...
  var controller = function ($scope, $state, $http, async, FileUploader, Project, Provision, Supporter, addressResolver) {
    $scope.$emit("projects", []);
    $scope.project = {
      name: "未命名專案"
    };
    ...
    $scope.addProvision = function () {
      ...
      uploader.onCompleteItem = function (item, response) {
        ...
        var photo = "/api/containers/" + container + "/download/" + name;
        undefined.scope.photos = [photo];
      };
    ...
    $scope.checkMap = function () {
      var address = [$scope.project.city, $scope.project.district, $scope.project.detailAddress].join("");
      if (address.length !== 0) {
    ...
    $scope.create = function () {
      async.series([function (callback) {
        ...
        Supporter.projects.create({ id: $scope.user.id }, $scope.project).$promise.then(function (project) {
        ...
        var tasks = $scope.provisions.map(function (p) {
          ...
          return function (callback) {
            Project.provisions.create({ id: $scope.project.id }, p).$promise.then(function () {
              return callback();
            });
          };
        });
        ...
        return $state.go("project", { id: $scope.project.id });
      });
    };
