...
(function () {
  ...
  var controller = function ($scope, $state, Project, Provision, Batch, async, Supporter) {
    ...
    Project.findOne({ filter: filter }).$promise.then(function (project) {
      ...
      $scope.project.provisions.forEach(function (p, index) {
    ...
    $scope.donate = function () {
      ...
      Project.batches.create({ id: $scope.project.id }, $scope.batch).$promise.then(function (batch) {
        var tasks = $scope.donatedProvisions.map(function (p, index) {
          return function (callback) {
            Batch.donatedProvisions.create({ id: batch.id }, p).$promise.then(function (dp) {
              return updateQuantity(dp, callback);
            });
          };
        });
        async.series(tasks, function () {
          console.log("done");
          $state.go("address", { projectId: $scope.project.id, batchId: batch.id });
        });
      });
    };
  };
